...
```bash
# macOS
brew install cmake conan yaml-cpp nlohmann-json googletest

# Ubuntu/Debian
sudo apt-get install cmake libyaml-cpp-dev nlohmann-json3-dev libgtest-dev
pip install conan
```
...
# prov_config.yaml.example  # 配置文件示例
```
...
scp output/prov_config.yaml.example user@tbox-device:/etc/tbox/prov_config.yaml
```
