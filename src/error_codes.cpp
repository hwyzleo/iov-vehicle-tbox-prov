...
static const std::map<ErrorCode, std::string> error_code_descriptions = {
    ...
    {ErrorCode::SE_UID_READ_FAILED, "SE UID读取失败/超时（SE在位）"},
    {ErrorCode::SE_MISSING_CONFIG_NOT_FOUND, "无SE且配置文件缺失/无对应UID"},
    {ErrorCode::SE_MISSING_PRODUCTION_FAIL_CLOSED, "生产环境无SE，禁止配置文件兜底"},
